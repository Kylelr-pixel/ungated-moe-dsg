...
def run_16head_comparison():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"\n=========================================================")
    print(f"16-HEAD, 8-ITERATION MOE COMPARISON TEST ON: {device.upper()}")
    print(f"=========================================================\n")
    
    batch_size, seq_len, hidden_dim, num_experts, top_k = 4, 32, 128, 16, 2
    ...
    print("--- Model A: Conditional Skipped-Head Calibration MoE (16 Experts) ---")
    ...
    for iteration in range(8):
        ...
        for i, expert in enumerate(experts_a):
            ...
            status = "ACTIVE" if head_tokens_assigned > 0 else "SKIPPED (Calibrated Grad)"
            print(f"  Head {i:2d}: Tokens = {int(head_tokens_assigned):3d} | Status = {status:28s} | Grad Norm = {grad_norm:.6f}")
        print("-" * 65)

    print("\n--- Model B: Industry Standard Top-K MoE + L_aux (16 Experts) ---")
    ...
    for iteration in range(8):
        ...
        for i, expert in enumerate(experts_b):
            ...
            grad_norm = expert.weight.grad.norm().item() if expert.weight.grad is not None else 0.0
            status = "ACTIVE" if assigned_count > 0 else "STARVED (Zero Grad)"
            print(f"  Head {i:2d}: Assignments = {assigned_count:3d} | Status = {status:26s} | Grad Norm = {grad_norm:.6f}")
        print(f"  Auxiliary Load Balancing Loss (L_aux): {aux_loss.item():.4f}")
        print("-" * 65)

if __name__ == "__main__":
    run_16head_comparison()
