...
class ScaledUnGatedFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, expert_outputs, gate_weights, active_mask, gamma=0.1):
        ctx.save_for_backward(gate_weights, active_mask)
        ctx.gamma = gamma
        return (expert_outputs * (gate_weights * active_mask)).sum(dim=-2)

    @staticmethod
    def backward(ctx, grad_output):
        gate_weights, active_mask = ctx.saved_tensors
        gamma = ctx.gamma
        grad_out_expanded = grad_output.unsqueeze(-2)
        
        dormant_mask = 1.0 - active_mask
        grad_expert_outputs = (grad_out_expanded * gate_weights * active_mask) + \
                              (grad_out_expanded * gamma * dormant_mask)
        
        grad_gate_weights = (grad_out_expanded * active_mask).sum(dim=-1, keepdim=True)
        return grad_expert_outputs, grad_gate_weights, None, None
...
class DomainSignatureGenerator(nn.Module):
    ...
    def forward(self, x):
        # Force FP32 precision for directional vector normalization
        x_fp32 = x.float()
...
# 4. DATA PIPELINE
# ==========================================
def get_dataset_loaders(batch_size=8, seq_len=128):
    ...
    def tokenize_text(dataset_split):
        text = "\n\n".join([t for t in dataset_split["text"] if len(t.strip()) > 0])
        tokens = torch.tensor(tokenizer.encode(text), dtype=torch.long)
        num_seqs = (len(tokens) - 1) // seq_len
        tot = num_seqs * seq_len
        inputs = tokens[:tot].view(num_seqs, seq_len)
        targets = tokens[1:tot + 1].view(num_seqs, seq_len)
    ...
    train_loader = tokenize_text(dataset["train"])
    val_loader = tokenize_text(dataset["validation"])
...
if __name__ == "__main__":
    ...
    # Modern PyTorch 2.x GradScaler setup
    scaler = torch.amp.GradScaler('cuda', enabled=(device == "cuda"))
    
    # Device-aware profiling activities
    ...
    print("\n=========================================================================================")
    print("Benchmark complete cleanly. (If using GPU studio, switch compute instance to CUDA).")
    print("=========================================================================================\n")